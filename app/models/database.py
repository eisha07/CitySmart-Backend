...
class ProjectVersionModel(Base):
    ...
    version_tag = Column(
        String, default="v1", nullable=False
    )  # e.g., "v1", "v2-amended", "v3-final"
    raw_text = Column(String, nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)

    # Relationships
    project = relationship("ProjectModel", back_populates="versions")
    document_insights = relationship(
...
class DocumentInsightModel(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    version_id = Column(
        Integer, ForeignKey("project_versions.id", ondelete="CASCADE"), nullable=False
    )
    ...
    embedding_vector = Column(
        JSON, nullable=True
    )  # Stored array format or vector mapping tracking line
...
class SimulationResultModel(Base):
    ...
    payload = Column(
        JSON, nullable=False
    )  # Stores scores, dynamic 10-persona configurations, ticker logs, and design fixes
    created_at = Column(DateTime, default=datetime.utcnow)
