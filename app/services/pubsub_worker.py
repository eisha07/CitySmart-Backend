...
class CloudPubSubWorkerLoop:
    ...
    async def process_background_simulation_event(self, project_id: str, context_chunks: list[str]):
        ...
        print(f"🚀 [Worker Engine] Starting multi-agent simulation loop for project: {project_id}")
        try:
            ...
            print(f"✅ [Worker Engine] Simulation successfully processed and saved for project: {project_id}")
            
        except Exception as e:
            print(f"🔴 [Worker Engine] Background processing failed for project {project_id}: {e}")
    ...
    def _pubsub_callback(self, message: pubsub_v1.subscriber.message.Message):
        ...
        try:
            ...
            if loop.is_running():
                asyncio.ensure_future(self.process_background_simulation_event(project_id, context_chunks))
            else:
                loop.run_until_complete(self.process_background_simulation_event(project_id, context_chunks))
    ...
    def start_listening(self):
        ...
        print(f"📡 [Worker Engine] Streaming event pull initialized on {self.subscription_path}...")
        streaming_pull_future = self.subscriber.subscribe(self.subscription_path, callback=self._pubsub_callback)
